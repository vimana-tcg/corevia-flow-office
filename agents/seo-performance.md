...
**LCP subparts** (TTFB, resource load delay, resource load time, element render delay) are now available in CrUX data (February 2025). See the `cwv-thresholds` reference for details.

## Tools

```bash
# PageSpeed Insights API (API key: возьми из .env / конфига проекта)
...
If Google API credentials are configured (возьми из .env / конфига проекта), prefer CrUX field data over Lighthouse lab data for CWV assessment:
```bash
