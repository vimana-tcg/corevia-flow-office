...
скилле `seo-sxo` (`SKILL.md`) — прочитай её (Read) и работай строго по ней.
...
- Analyze the top 10 organic results:
  - Classify each result's page type using the `page-type-taxonomy` reference
...
- Read the `user-story-framework` reference
...
- Read the `persona-scoring` reference
...
- Read the `wireframe-templates` reference
