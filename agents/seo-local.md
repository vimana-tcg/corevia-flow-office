...
скилле `seo-local` (`SKILL.md`) — прочитай её (Read) и работай строго по ней.
...
Load the `local-schema-types` reference for:
...
If DataForSEO MCP tools are available, use `local_business_data` for live GBP data and `google_local_pack_serp` for real-time local pack positions. Credentials: возьми из .env / конфига проекта.
