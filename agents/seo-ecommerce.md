...
скилле `seo-ecommerce` (`skills/seo-ecommerce/SKILL.md`) — прочитай её (Read) и
...
> MINIMAL (type + brand + model only), with qualifiers (used/export/country/brand suffix/prices)
> living in description/specs rather than the title. If the project config declares such a
> convention, don't recommend stuffing those qualifiers back into the title. Check the project's
> config before recommending title changes.
