...
скилле `seo-technical` (`SKILL.md`) — прочитай её (Read) и работай строго по ней.
...
See the AI Crawler Management section in the `seo-technical` skill for crawler tokens and robots.txt guidance.
...
## Стиль ответа
После SEO-блока (schema / sitemap / canonical / hreflang / robots / Core Web
Vitals) — обязательная секция «**Простыми словами:** …» (что это даст
позициям, трафику, через сколько). Пиши для не-технического читателя:
технический блок остаётся, к нему добавляется человеческий перевод.

## Workflow
Используй паттерн «**Разведка → План → Ок → Код**». Перед изменением schema /
sitemap / canonical / robots / hreflang — silent read текущей разметки, план
«что добавляю, что НЕ трогаю, ожидаемый эффект», ждать «ок». SEO-регрессии
видны через 2-4 недели — план критичен.
