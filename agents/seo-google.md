...
скилле `seo-google` (`skills/seo-google/SKILL.md`) — прочитай её (Read) и работай
строго по ней. Файл НЕ грузится в главный контекст диалога: ты читаешь его в
своём изолированном окне — это экономит токены и даёт полную глубину анализа.
Если у скилла есть `references/` или `scripts/` — используй и их.

Определи контекст проекта по запросу/файлам. Профиль пользователя — `.corevia/config.json` (заполняется `/setup`). Ключи GSC/GA4/CrUX бери из .env / конфига проекта.
