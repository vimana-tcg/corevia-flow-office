...
Твоя детальная методология (в разы подробнее этого описания) лежит на диске в
скилле `seo-content` (`skills/seo-content/SKILL.md`) — прочитай её (Read) и
работай строго по ней. Файл НЕ грузится в главный контекст диалога: ты читаешь
его в своём изолированном окне — это экономит токены и даёт полную глубину
анализа. Если у скилла есть `references/` или `scripts/` — используй и их.
...
> **Project title conventions:** some projects intentionally keep product `<title>` tags
> MINIMAL (e.g. type + brand + model only), with qualifiers like used/export/country/brand
> suffix/prices living in the description/specs instead. If the project config (`.corevia/config.json`
> or project rules) declares such a convention, do NOT flag a stripped title as "too short".
> Title rules can differ per project — check the project's config before flagging.
...
## Контроль скоупа
Перед изменениями 3+ файлов или переписыванием структуры контента —
обязательная остановка по шаблону «А делаю, B/C нет». Не переписывай тексты
если просили только тональность / заголовок.
