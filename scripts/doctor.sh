...
echo "Видео-монтаж (софт):"
command -v ffmpeg >/dev/null 2>&1 && echo "  ✅ ffmpeg" || echo "  ⬜ ffmpeg — нужен для монтажа (запустите scripts/setup-video.sh)"
python3 -c "import PIL" 2>/dev/null && echo "  ✅ Pillow (обложки)" || echo "  ⬜ Pillow — pip3 install Pillow"
echo ""
echo "ℹ️ Canva подключается как MCP (не ключом) — см. skills/setup/references/api-setup.md"
echo "ℹ️ Видео-софт детально: bash scripts/setup-video.sh"
